...
RUN dnf upgrade -y && \
    ...
    curl -fsSL https://go.dev/dl/go1.24.13.linux-${TARGETARCH}.tar.gz | tar -C /usr/local -xz && \
    go mod download
...
RUN if [ "$TARGETARCH" = "amd64" ]; then \
      GOEXPERIMENT=boringcrypto CGO_ENABLED=1 GOOS=linux go build -o /go/bin/ratelimit -ldflags="-w -s" -v github.com/replicon/ratelimit/src/service_cmd && \
      ...
      CGO_ENABLED=1 GOOS=linux go build -o /go/bin/ratelimit -ldflags="-w -s" -v github.com/replicon/ratelimit/src/service_cmd && \
      CGO_ENABLED=1 GOOS=linux go build -o /go/bin/ratelimit_config_check -ldflags="-w -s" -v github.com/replicon/ratelimit/src/config_check_cmd; \
    fi
